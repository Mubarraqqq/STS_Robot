...
class VoiceAssistant:
    ...
    def clean_speech_with_corrections(self, text: str) -> str:
        """
        Clean speech text by removing self-corrections and false starts.
        
        Args:
            text: Raw transcribed text
            
        Returns:
            Cleaned text with corrections removed
        """
        if not text:
            return text
        ...
        correction_patterns = [
            # "no wait", "no sorry", "actually", "I mean"
            r'\b(no\s+wait|no\s+sorry|actually|i\s+mean)\b[,\s]*',
            # "sorry I meant", "let me correct that"
            r'\b(sorry\s+i\s+meant|let\s+me\s+correct\s+that)\b[,\s]*',
            # Repeated words (stuttering) - "the the", "and and"
            r'\b(\w+)\s+\1\b',
            # False starts with "uh", "um", "er"
            r'\b(uh|um|er|ah)\b[,\s]*',
            # "no that's wrong" type patterns
            r'\b(no\s+that\'?s\s+wrong|that\'?s\s+not\s+right)\b[,\s]*',
        ]
        
        cleaned_text = text.lower()
        
        for pattern in correction_patterns:
            # For repeated words, replace with single instance
            if r'\1' in pattern:
        ...
        if cleaned_text != text.lower():
            logger.info(f"Speech cleaned: '{text}' -> '{cleaned_text}'")
    ...
    def start_background_listening(self):
        ...
        if not self.background_listening:
            ...
            self.listening_thread = threading.Thread(target=self._background_listener, daemon=True)
    ...
    def _background_listener(self):
        """Background thread that listens for interruptions."""
        recognizer = sr.Recognizer()
        recognizer.energy_threshold = 500  # Higher threshold for interruptions
        recognizer.pause_threshold = 0.5   # Shorter pause for quick interruptions
        ...
        with sr.Microphone() as source:
            ...
            while self.background_listening:
                try:
                    # Only listen when we're speaking
                    if self.is_speaking:
                        # Quick, short listening for interruptions
                        audio = recognizer.listen(source, timeout=0.1, phrase_time_limit=2)
                        
                        try:
                            # Quick recognition check
                            text = recognizer.recognize_google(audio)
                            if text and len(text.strip()) > 2:  # Minimum viable interruption
                                logger.info(f"Interruption detected: {text}")
                                self.interrupt_detected = True
                                self.audio_queue.put(text)
                                break
                        except sr.UnknownValueError:
                            pass  # No clear speech detected
                        except sr.RequestError:
                            pass  # Recognition service error
                    else:
                        time.sleep(0.1)  # Small delay when not speaking
                        
                except sr.WaitTimeoutError:
                    continue  # Normal - no speech detected
    ...
    def speak(self, text: str):
        ...
        try:
            ...
            for i, sentence in enumerate(sentences):
                ...
                # Small pause between sentences to allow interruption
    ...
    def handle_interruption(self):
        ...
        try:
            # Get the interruption text
            interruption_text = ""
            try:
                interruption_text = self.audio_queue.get_nowait()
            except queue.Empty:
                # If no text captured, listen for more
                logger.info("Listening for complete interruption...")
                interruption_text = self.listen_for_command(timeout=5)
            
            if interruption_text:
                # Clean the interruption text
                cleaned_text = self.clean_speech_with_corrections(interruption_text)
                
                if cleaned_text:
                    logger.info(f"Processing interruption: {cleaned_text}")
                    
                    # Check if it's a stop command
                    if any(word in cleaned_text.lower() for word in ['stop', 'wait', 'hold on', 'pause']):
                        self.speak("Okay, I'll wait.")
                        return
                    
                    # Process the interruption as a new query
                    self.last_interaction_time = time.time()
                    ai_response = self.get_ai_response(cleaned_text)
                    self.speak(ai_response)
                    
        except Exception as e:
            logger.error(f"Interruption handling error: {e}")
            self.speak("Sorry, what did you say?")

    def listen_for_command(self, timeout: int = 12, phrase_limit: int = 20) -> Optional[str]:
        """
        Listen for user command with improved sensitivity and self-correction handling.
        ...
        Returns:
            Cleaned transcribed text or None if failed
        """
        try:
            with sr.Microphone() as source:
                ...
                self.recognizer.pause_threshold = 1.5  # Even longer pause for corrections
                self.recognizer.phrase_threshold = 0.2  # Shorter minimum audio length
                self.recognizer.non_speaking_duration = 1.0  # Wait longer after speech stops
                ...
                try:
                    ...
                    logger.info(f"User said (raw): {text}")
                    
                    # Clean the speech for corrections
                    cleaned_text = self.clean_speech_with_corrections(text)
                    logger.info(f"User said (cleaned): {cleaned_text}")
    ...
    def get_ai_response(self, user_input: str) -> str:
        ...
        try:
            ...
            messages=[
                    ...
                    "role": "system",
                    "content": (
                        "Your name is Bruce. You are a helpful voice assistant designed to answer questions clearly and conversationally."
                    
                    "CORE BEHAVIOR:"
                    "- Respond in simple, clear English that a 12-year-old can easily understand"
                    "- Keep responses concise (1-3 sentences for simple questions, longer only when necessary for clarity)"
                    "- Be friendly and natural, as if continuing a conversation\n- Prioritize being helpful and accurate over being brief"
                    
                    "FORMATTING RULES:"
                    "- No emojis or decorative symbols"
                    "- No bullet points or numbered lists in responses"
                    "- Use plain punctuation only (periods, commas, question marks)"
                    "- Speak in complete sentences"
                    
                    "HANDLING REQUESTS:"
                    "- If you don't know something, say so honestly: 'I don't know that, but here's what I can tell you...'"
                    "- For inappropriate or harmful requests, politely decline: 'I can't help with that, but I'd be happy to help with something else'"
                    "- For complex topics, break them down into simple terms without talking down to the user"
                    "- Remember context from earlier in the conversation when relevant"
                    
                    "WHEN TO BE LONGER:"
                    "- Important safety information"
                    "- Step-by-step explanations that need detail"
                    "- When brevity would make the answer confusing or incomplete"
                    
                    "STAY IN CHARACTER:"
                    "- You're Bruce, a knowledgeable but approachable assistant"
                    "- Respond naturally without announcing what you're doing"
                    "- If uncertain about recent events, acknowledge your knowledge limitations"
                    )
                    },
